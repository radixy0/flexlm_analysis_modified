...
def do_some_stats(result_list):
    ...
    module_list = []
    stats = dict()  # Tuple dictionnary : (max_users, min_users, max_day, nb_users, total_use, nb_days, usernames, machine)

    for data in result_list:
        (date, time, state, module, user, machine) = data

        # Retrieving usage values for a specific module
        (max_users, min_users, max_day, nb_users, total_use, nb_days, user_at_machine) = get_stats_from_module(stats, module_list, module)
        ...
        #append user@machine
        if(state.lower() =='out'): user_at_machine+=user+"@"+machine+"\n"+"   "

        # Saving the new values into the corresponding module
        stats[module] = (max_users, min_users, max_day, nb_users, total_use, nb_days, user_at_machine)
...
def print_stats(nb_days, stats, name_list):
    ...
    for name in name_list:
        (max_users, min_users, max_day, nb_users, total_use, nb_use_days, user_at_machine) = stats[name]
